...
class Twitter(object):
    ...
    def post(self, message, image_path):
        ...
        if req_media.status_code != 200:
            self.logger.error(
                'Media Upload failed. Status Code:{0}, Detail:{1}'.format(req_media.status_code, req_media.text))
            return False
        ...
        if req_text.status_code != 200:
            self.logger.error('Tweet failed. Status Code:{0}, Detail:{1}'.format(req_text.status_code, req_text.text))
            return False
