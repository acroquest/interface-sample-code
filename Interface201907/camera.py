...
class DetectionCamera:
    ...
    def run(self):
        ...
        while True:
            ...
            if matched_name is not None:
                ...
                self._crop_center(matched_box, 'tmp.jpg', 'crop.jpg')
    ...
    def _crop_center(self, matched_box, original_image_path, crop_image_path):
        """
        matched_boxと中央が合う位置で、original_image_pathの画像を切り取り、その結果をcrop_image_pathのファイルに保存する。
        :param matched_box: 検知枠
        ...
        xmin, xmax, ymin, ymax = (xmin_n * org_width, xmax_n * org_width,
                                  ymin_n * org_height, ymax_n * org_height)

        # (3) 検知枠と中心点が合うような切り取り枠の位置を求める。
        ...
        # (5) 切り取り枠で、元画像を切り取った結果の画像を生成し、保存する。
